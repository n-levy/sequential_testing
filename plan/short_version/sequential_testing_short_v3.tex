...
\begin{enumerate}[nosep]
  ...
  \item \textbf{The Eppo Solution} --- How a modern platform solves it,
        including the recommended \textbf{hybrid approach}.
  \item \textbf{DIY Alternatives} --- What to do if you don't have Eppo,
        including how to implement the hybrid approach yourself.
\end{enumerate}
...
\textbf{What changed from v2:}
Added the hybrid approach (sequential testing for guardrail KPIs only,
fixed-horizon for the primary KPI) to Act~2 and Act~3, with implementation
details and Python code in Appendix~A.
...
% ──────────────────────────────────────────────────────────────────────────────
\subsection{The Hybrid Approach (Recommended)}

The full sequential pipeline described above is powerful but comes with a
cost: the sequential CI is always wider than a fixed-horizon CI (the ``price
of peeking'').  For the \textbf{primary KPI} --- the metric you are trying to
improve --- this wider CI means you need more data to detect the same effect
size.

In many organisations, the primary goal of continuous monitoring is not to
\emph{declare a winner early} but to \textbf{catch damage to guardrail
metrics quickly}.  This observation leads to a practical hybrid design
that combines the best of both worlds.

\subsubsection*{The idea}

\begin{intuition}
\textbf{Use sequential testing where it matters most --- early stopping for
safety --- and fixed-horizon testing where it is most efficient --- the
primary metric.}

Most experiments are not designed to be stopped early for success.  You have
a planned duration (say 2--4 weeks), and you want to know at the end whether
the treatment improved your primary KPI.  What you \emph{do} want is an
early warning system: if the treatment is causing real harm to a critical
metric (revenue, error rate, latency), you need to know \emph{now}, not in
4 weeks.

The hybrid approach splits your metrics into two categories:
\begin{itemize}[nosep]
  \item \textbf{Guardrail KPIs} (revenue, error rate, latency, etc.):
        monitored with a \textbf{sequential CI}.  You can check these every
        day or even continuously.  If the CI goes fully below zero (or above,
        for metrics where ``up'' is bad), abort the experiment immediately.
  \item \textbf{Primary KPI} (the metric you are trying to improve):
        analysed with a \textbf{standard fixed-horizon CI} at the
        pre-planned end of the experiment.  No peeking penalty.  Full
        statistical power.
\end{itemize}
\end{intuition}
...
The peeking problem (Act~1) is a problem of \textbf{repeated decision-making
on the same metric}.  In the hybrid approach:

\begin{itemize}[nosep]
  \item The primary KPI is tested \textbf{exactly once} (at the end), so
        there is no multiplicity issue and no need for a wider CI.
  \item The guardrail KPIs use a sequential CI, which is wider --- but this
        is the right trade-off: you are not trying to detect a subtle
        improvement in revenue, you are trying to detect \emph{catastrophic
        damage}.  The wider CI is still narrow enough to catch a large
        negative effect quickly.
\end{itemize}

\subsubsection*{The decision rules}

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{} p{4cm} l p{5.5cm} @{}}
\toprule
\textbf{Metric type} & \textbf{When checked} & \textbf{Decision rule} \\
\midrule
Guardrail KPI (e.g.\ revenue, error rate)
  & Continuously (sequential CI)
  & \textcolor{red}{\textbf{ABORT}} if the sequential CI is entirely on the
    harmful side of zero at any time during the experiment. \\[6pt]
Primary KPI (e.g.\ conversion rate)
  & Once at the end (fixed-horizon CI)
  & \textcolor{green!60!black}{\textbf{SHIP}} if the standard CI excludes
    zero.  \textcolor{gray}{\textbf{NO EFFECT}} otherwise. \\
\bottomrule
\end{tabular}
\end{center}

\subsubsection*{What you gain}

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{} l c c @{}}
\toprule
& \textbf{Full sequential} & \textbf{Hybrid} \\
\midrule
Primary KPI CI width & Wider ($\sim\!10$--$40\%$) & Standard (no penalty) \\
Primary KPI power & Reduced & Full \\
Guardrail protection & Continuous & Continuous \\
Early stopping for success & Yes & No \\
Early stopping for harm & Yes & Yes \\
...
\begin{plainlanguage}
\textbf{The trade-off is simple:}
\begin{itemize}[nosep]
  \item You give up the ability to stop the experiment early because the
        primary KPI looks good.
  \item You gain full statistical power for the primary KPI --- your CI is
        as tight as it can be, so you need fewer users to detect a given
        effect size.
  \item You keep the ability to abort early if a guardrail metric is being
        damaged --- the only form of peeking that most teams actually need.
\end{itemize}
\end{plainlanguage}

\subsubsection*{When to use the hybrid approach}

The hybrid approach is the right choice when:
\begin{enumerate}[nosep]
  \item You have a \textbf{fixed experiment duration} dictated by product
        cycles, business calendars, or sample-size calculations.
  \item You care most about \textbf{detecting a small positive effect} on
        the primary KPI (full power matters).
  \item You want a \textbf{safety net} for metrics that should never degrade
        (revenue, latency, error rates).
  \item Your team prefers \textbf{simple interpretation}: ``check the
        guardrails daily, read the primary result at the end.''
\end{enumerate}

The full sequential approach is better when you genuinely want to stop early
for success (e.g.\ a high-risk launch where shipping one week earlier has
significant business value).

\begin{keytakeaway}
\textbf{The hybrid approach in one sentence:}
...
\textbf{This is the recommended approach for most A/B testing programmes.}
\end{keytakeaway}
...
or a few lines of code --- and compares them head-to-head with Eppo.  It also
shows how to implement the hybrid approach from scratch.
...
% ──────────────────────────────────────────────────────────────────────────────
\subsection{Implementing the Hybrid Approach Without Eppo}

Act~2 described the hybrid approach: sequential CI on guardrail KPIs for
early abort, standard CI on the primary KPI at the end.  Here is how to
implement it without a platform, using any of the three methods above.

\subsubsection*{Step 1: Classify your metrics}

Before the experiment starts, split your metrics into two lists:

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{} l l l @{}}
\toprule
\textbf{Category} & \textbf{Examples} & \textbf{Testing method} \\
\midrule
Primary KPI & Conversion rate, sign-ups & Fixed-horizon ($z = 1.96$) \\
Guardrail KPIs & Revenue, error rate, latency, crash rate &
  Sequential (any of the 3 methods) \\
\bottomrule
\end{tabular}
\end{center}

\subsubsection*{Step 2: Choose your guardrail monitoring schedule}

Decide how often you will check guardrail KPIs:
\begin{itemize}[nosep]
  \item \textbf{Weekly} (recommended for most teams): $K = $ number of weeks
        in the experiment.
  \item \textbf{Daily}: $K = $ number of days.  Use O'Brien--Fleming to
        avoid the harsh Bonferroni penalty at large $K$.
\end{itemize}

\subsubsection*{Step 3: Set guardrail significance levels}

You are testing \emph{multiple} guardrail KPIs, each with $K$ peeks.  To
control the overall false positive rate across all guardrails:

\textbf{Option A (simplest):}  Use a Bonferroni correction \emph{across
guardrails}.  If you have $G$ guardrail KPIs, set
$\alpha_{\text{guardrail}} = \alpha / G$ for each one.

\textbf{Example:} 3 guardrails ($G = 3$), $\alpha = 0.05$, 4 weekly peeks
($K = 4$):
\[
  \alpha_{\text{per guardrail}} = \frac{0.05}{3} \approx 0.0167
\]
Then apply Bonferroni within each guardrail:
\[
  z^* = \Phi^{-1}\!\bigl(1 - \tfrac{0.0167}{2 \times 4}\bigr)
  = \Phi^{-1}(0.9979) \approx 2.86
\]

\textbf{Option B (tighter):} Use O'Brien--Fleming within each guardrail,
with $\alpha_{\text{per guardrail}} = \alpha / G$.  The final-peek threshold
will be close to $z_{\alpha/(2G)} \approx 2.39$ (for $G = 3$), which is
much less wasteful than Bonferroni's constant 2.86.

\subsubsection*{Step 4: Run the experiment}

At each scheduled peek:
\begin{enumerate}[nosep]
  \item For \textbf{each guardrail KPI}: compute the CI using your chosen
        sequential method (Bonferroni, Pocock, or O'Brien--Fleming) with
        the per-guardrail $\alpha$.
  \item If \textbf{any} guardrail CI is entirely on the harmful side of
        zero: \textcolor{red}{\textbf{ABORT}} the experiment.
  \item Otherwise: continue.
\end{enumerate}

At the \textbf{end} of the experiment:
\begin{enumerate}[nosep]
  \item For the \textbf{primary KPI}: compute a standard CI with the full
        $\alpha$ (or your preferred primary $\alpha$):
        \[
          \text{CI}_{\text{primary}} = \hat{\tau} \;\pm\;
          z_{\alpha/2} \cdot \text{SE}
        \]
  \item \textcolor{green!60!black}{\textbf{SHIP}} if the CI excludes zero.
        Otherwise: no significant effect.
\end{enumerate}

\subsubsection*{Summary: the complete recipe}

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{} r p{10cm} @{}}
\toprule
\textbf{When} & \textbf{What to do} \\
\midrule
Before experiment
  & Classify metrics (primary vs.\ guardrail).  Choose $K$, $\alpha$, $G$.
    Compute per-guardrail $\alpha_g = \alpha / G$ and critical values for
    your chosen method. \\[4pt]
At each peek $k$
  & For each guardrail: compute $Z_k$ and compare against the sequential
    threshold.  Abort if any guardrail is significantly harmed. \\[4pt]
At scheduled end
  & Compute the primary KPI CI with $z_{0.025} = 1.96$ (no penalty).
    Ship or don't ship based on this single test. \\
\bottomrule
\end{tabular}
\end{center}

\begin{keytakeaway}
\textbf{The hybrid approach without Eppo:}
\begin{itemize}[nosep]
  \item Apply Bonferroni across your $G$ guardrail KPIs:
        $\alpha_g = \alpha / G$.
  \item Within each guardrail, use O'Brien--Fleming (best) or Bonferroni
        (simplest) across $K$ peeks.
  \item Test the primary KPI once at the end with a standard CI --- no
        correction needed.
  \item This is the \textbf{recommended approach} for teams without a
        sequential testing platform.
\end{itemize}
\end{keytakeaway}
...
2 & The Eppo Solution
  & A sequential CI is valid at every moment.  The \textbf{hybrid approach}
    uses sequential testing only for guardrail KPIs (early abort) and a
    standard CI for the primary KPI (full power). \\
3 & DIY Alternatives
  & Bonferroni (simplest), Pocock, O'Brien--Fleming --- three ways to
    peek safely without a platform.  The hybrid approach works with any
    of them. \\
\bottomrule
\end{tabular}
\end{center}
...
Hybrid (recommended) & \textbf{No penalty on primary KPI}; sequential
  penalty on guardrails only \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}[nosep]
  ...
  \item A hybrid approach implementation (Act~2 \& Act~3).
\end{enumerate}
...
% ──────────────────────────────────────────────────────────────────────────────
\subsection{Implementation: Hybrid Approach}

This snippet implements the complete hybrid workflow: O'Brien--Fleming on
guardrail KPIs at each peek, standard CI on the primary KPI at the end.
...
def hybrid_experiment(
    primary_ctrl: np.ndarray,
    primary_trt: np.ndarray,
    guardrail_data: dict[str, tuple[np.ndarray, np.ndarray]],
    K: int = 4,
    alpha: float = 0.05,
    guardrail_direction: dict[str, str] | None = None,
) -> dict:
    """Run a hybrid sequential/fixed-horizon experiment.

    Args:
        primary_ctrl, primary_trt:
            Full arrays for the primary KPI.
        guardrail_data:
            Dict mapping guardrail name -> (ctrl_array, trt_array).
        K: Number of planned peeks for guardrails.
        alpha: Overall significance level.
        guardrail_direction:
            Dict mapping guardrail name -> 'lower' or 'upper'.
            'lower' means harm = treatment < control (default).
            'upper' means harm = treatment > control
            (e.g. error_rate).

    Returns:
        Dict with 'aborted', 'abort_reason', 'abort_peek',
        'primary_result'.
    """
    if guardrail_direction is None:
        guardrail_direction = {}

    G = len(guardrail_data)
    alpha_per_guardrail = alpha / G if G > 0 else alpha
    n_total = len(primary_ctrl)
    peek_every = n_total // K

    # --- Phase 1: Monitor guardrails sequentially ---
    for k in range(1, K + 1):
        n = k * peek_every

        for name, (g_ctrl, g_trt) in guardrail_data.items():
            c, t = g_ctrl[:n], g_trt[:n]
            tau = t.mean() - c.mean()
            se = np.sqrt(
                c.var(ddof=1) / len(c)
                + t.var(ddof=1) / len(t)
            )
            # OBF critical value with per-guardrail alpha
            c_k = obf_critical_value(
                k, K, alpha_per_guardrail
            )

            direction = guardrail_direction.get(
                name, "lower"
            )
            # Check for harm only (one-sided)
            if direction == "lower" and tau / se < -c_k:
                return {
                    "aborted": True,
                    "abort_reason": f"{name} harmed",
                    "abort_peek": k,
                    "guardrail_z": tau / se,
                    "guardrail_threshold": -c_k,
                    "primary_result": None,
                }
            if direction == "upper" and tau / se > c_k:
                return {
                    "aborted": True,
                    "abort_reason": f"{name} harmed",
                    "abort_peek": k,
    ...
    # --- Phase 2: Analyse primary KPI at end ---
    tau = primary_trt.mean() - primary_ctrl.mean()
    se = np.sqrt(
        primary_ctrl.var(ddof=1) / len(primary_ctrl)
        + primary_trt.var(ddof=1) / len(primary_trt)
    )
...
# --- Example: 4-week experiment, 3 guardrails ---
rng = np.random.default_rng(42)
N = 4000

# Primary KPI: small positive effect
primary_ctrl = rng.normal(100, 30, N)
primary_trt = rng.normal(101, 30, N)

# Guardrails: revenue (no harm), error rate (no harm),
#             latency (treatment causes harm!)
guardrails = {
    "revenue": (
        rng.normal(50, 15, N),     # control
        rng.normal(50, 15, N),     # treatment (no effect)
    ),
    "error_rate": (
        rng.normal(0.02, 0.01, N),
        rng.normal(0.02, 0.01, N),  # no effect
    ),
    "latency_ms": (
        rng.normal(200, 50, N),
        rng.normal(220, 50, N),     # +20ms harm!
    ),
}

result = hybrid_experiment(
    primary_ctrl, primary_trt,
    guardrails,
    K=4,
    guardrail_direction={
        "revenue": "lower",       # harm = decrease
        "error_rate": "upper",    # harm = increase
        "latency_ms": "upper",    # harm = increase
    },
)

if result["aborted"]:
    print(f"ABORTED at peek {result['abort_peek']}: "
          f"{result['abort_reason']}")
    print(f"  Z = {result['guardrail_z']:.3f}, "
          f"threshold = {result['guardrail_threshold']:.3f}")
else:
    pr = result["primary_result"]
    sig = "SIGNIFICANT" if pr["significant"] else "not significant"
    print(f"Experiment completed. Primary KPI: {sig}")
    print(f"  tau = {pr['tau_hat']:.3f}, "
...
Expected output (approximately):
\begin{verbatim}
ABORTED at peek 2: latency_ms harmed
  Z = 3.876, threshold = 3.226
\end{verbatim}
The experiment is aborted at peek~2 (50\% of data) because the latency
guardrail detected a significant increase.  The primary KPI is never
analysed --- the experiment was stopped for safety.

If the latency harm were removed (e.g.\ \texttt{rng.normal(200, 50, N)} for
treatment), the experiment would run to completion and the primary KPI
result would be reported with a standard CI (no peeking penalty).
\end{plainlanguage}
