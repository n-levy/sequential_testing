...
export function CoinFlipMeanSim({
  ...
  showAnalogy = true,
  analogyNote = '',
}: CoinFlipMeanSimProps & { showAnalogy?: boolean, analogyNote?: string }) {
  ...
  // Clamp bias to [-0.5, 0.5]
  const clampedBias = Math.max(-0.5, Math.min(0.5, bias))
  // Clamp pHeads to [0, 1]
  const pHeads = Math.max(0, Math.min(1, 0.5 + clampedBias))
  ...
  useEffect(() => {
    ...
    // Y-axis: include all values of the mean and all CI bands, guaranteed
    let allBounds: number[] = [];
    for (let i = 0; i < traj.means.length; i++) {
      const v = traj.means[i] - 0.5;
      allBounds.push(v);
      for (const layer of layers) {
        const band = bands[layer];
        if (band && band[i] != null && isFinite(band[i])) {
          allBounds.push(v - band[i]);
          allBounds.push(v + band[i]);
        }
      }
    }
  ...
  return (
    <div className="bg-white border border-neutral-300 rounded-lg p-4 my-6">
      ...
      <div className="mb-3 text-sm text-blue-900 font-semibold">
        Coin bias: <span className="font-mono">{clampedBias >= 0 ? '+' : ''}{clampedBias.toFixed(2)}</span> (P(heads) = {pHeads.toFixed(2)})
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
        ...
        <div>
          ...
          <input
            ...
            value={clampedBias} onChange={e => setBias(parseFloat(e.target.value))}
            className="w-full"
          />
        </div>
      </div>
